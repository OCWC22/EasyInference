...
def _extract_json_from_output(output: str, model: str) -> dict:
    ...
    # Try to find JSON object in raw text — get the LARGEST one (trace, not headers)
    return _parse_trace_json(output)


def _parse_trace_json(text: str) -> dict:
    """Extract the largest JSON object from text (handles Codex/Claude noise)."""
    ...
    # Find ALL valid JSON objects and return the largest one with trace_id
    candidates = []
    i = 0
    while i < len(text):
        if text[i] == "{":
    ...
    # Prefer objects with trace_id, then largest
    with_trace = [c for c in candidates if "trace_id" in c]
    if with_trace:
        return max(with_trace, key=lambda c: len(json.dumps(c)))
