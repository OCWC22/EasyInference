    ...
    def process(self, ctx: PipelineContext) -> None:
        ...
        if ctx.forced_engine != 'auto':
            ...
            if is_amd:
                ...
                if is_hopper:
                    ...
                    ctx.engine_type = EngineType.SGLANG
                    ctx.reasoning_trace.append(
                        'HardwareNode: Selected SGLang for coding (RadixAttention).'
                    )
                else:
                    ctx.engine_type = EngineType.VLLM
                    ctx.reasoning_trace.append(
                        'HardwareNode: Selected vLLM for NVIDIA general workloads.'
                    )
        ...
        if ctx.model.params_total_b <= 13 and ctx.gpu.memory_gb >= 40:
            ...
            if is_amd and ctx.model.model_type == 'moe':
                ...
                ctx.reasoning_trace.append(
                    'HardwareNode: Downgrading AMD MoE to BF16 '
                    '(rocprofv3 flagged FP8 decode regression).'
                )
            else:
                ctx.precision = PrecisionSpec(weights='fp8', activations='fp8', kv_cache='fp8_e4m3')
                ctx.reasoning_trace.append(
                    'HardwareNode: Native FP8 support detected; routing to FP8 weights.'
                )
...
class ModelNode(DAGNode):
    ...
    def process(self, ctx: PipelineContext) -> None:
        ctx.reasoning_trace.append(
            f"ModelNode: Analyzing {ctx.model.name} ({ctx.model.params_total_b}B total, "
            f"{ctx.model.model_class.value})."
        )
        ...
        if (
            ...
            ctx.reasoning_trace.append(
                'ModelNode: Forcing TP=1 for small model on AMD due to RCCL overhead '
                'compared to NCCL.'
            )
            tp = 1
        else:
...
class WorkloadNode(DAGNode):
    ...
    def process(self, ctx: PipelineContext) -> None:
        ...
        if ctx.workload == WorkloadMode.CODING:
            ...
            kv_bpt = ctx.model.kv_cache_bytes_per_token(
                'fp8' if ctx.gpu.fp8_support else 'fp16'
            ) * ctx.model.layers
        ...
        if is_hopper and ctx.engine_type == EngineType.VLLM:
            ...
            if is_h200:
                ctx.reasoning_trace.append(
                    'WorkloadNode: H200 141GB HBM3e @ 4.8 TB/s — most workloads fit '
                    'GPU-resident without KV offloading.'
                )
        ...
        if is_blackwell and multi_gpu and ctx.workload in (
...
class TelemetryNode(DAGNode):
    ...
    def process(self, ctx: PipelineContext) -> None:
        ctx.reasoning_trace.append(
            'TelemetryNode: Configured Prometheus /metrics scraper and Grafana '
            'KV cache capacity alerts.'
        )
...
class CompilerNode(DAGNode):
    ...
    def process(self, ctx: PipelineContext) -> None:
        ...
        ctx.reasoning_trace.append(
            'CompilerNode: Successfully bound DAG context to final EngineConfig.'
        )
