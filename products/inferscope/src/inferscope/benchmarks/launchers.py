...
class LaunchReadinessProbe(BaseModel):
    """How to verify a launched component is ready to serve."""
...
class LaunchComponent(BaseModel):
    ...
    name: str
    role: str
    kind: Literal["engine", "router", "proxy", "cache"]
    ...
    readiness: LaunchReadinessProbe | None = None
...
# Legacy: _simm_config_file and _lmcache_simm_config_files removed.
# SiMM is not a supported production backend. For disaggregated KV transfer,
# use Dynamo 1.0 + NIXL (see _dynamo_config_files and dynamo launcher branch).


def _dynamo_config_files(
    host: str,
    ports: dict[str, int],
    *,
    request_id_headers: list[str] | None = None,
) -> list[GeneratedFile]:
    """Generate a Dynamo 1.0 deployment config with concrete worker/router wiring."""
    rid_headers = request_id_headers or ["X-Request-ID"]
    content = f"""# Dynamo 1.0 deployment configuration
# Generated by InferScope benchmark stack planner
version: '1.0'

server:
  host: {host}
  api_port: {ports['primary']}
  metrics_port: {ports['router']}

routing:
  mode: kv_aware
  session_header: X-Session-ID
  request_id_headers: {rid_headers}

workers:
  prefill_pool:
    - name: prefill-0
      backend: vllm
      endpoint: http://{host}:{ports['prefill']}
      metrics_endpoint: http://{host}:{ports['prefill']}/metrics
  decode_pool:
    - name: decode-0
      backend: vllm
      endpoint: http://{host}:{ports['decode']}
      metrics_endpoint: http://{host}:{ports['decode']}/metrics

nixl:
  enabled: true
  bind_host: {host}
  transport: auto  # Uses NVLink/RDMA when available, falls back to TCP
...
"""
    return [
        GeneratedFile(
            path="configs/dynamo-config.yaml",
            content=content,
            description=(
                "Dynamo 1.0 declarative configuration with concrete worker endpoints, "
                "KV-aware routing, NIXL transport, and SLO targets."
            ),
        )
    ]
...
def _render_start_script(
    component_scripts: list[tuple[str, str]],
    readiness_probes: dict[str, LaunchReadinessProbe] | None = None,
) -> str:
    """Render a start-all.sh that launches components in order with readiness polling."""
    ...
    lines = [
        ...
        '  echo "waiting for ${name} to become ready (${kind} ${target}, timeout=${timeout}s)..."',
        '  while (( SECONDS < deadline )); do',
        '    if [[ "$kind" == "http" ]]; then',
        '      if curl -sf -o /dev/null --max-time 2 "$target" 2>/dev/null; then',
        '        echo "${name} is ready"',
        "        return 0",
        "      fi",
        '    elif [[ "$kind" == "tcp" ]]; then',
        '      local tcp_host="${target%%:*}"',
        '      local tcp_port="${target##*:}"',
        '      if bash -c "echo >/dev/tcp/${tcp_host}/${tcp_port}" 2>/dev/null; then',
        '        echo "${name} is ready"',
        "        return 0",
        "      fi",
        "    fi",
        '    sleep "$interval"',
        "  done",
        '  echo "WARNING: ${name} readiness timed out after ${timeout}s"',
        "  return 1",
        "}",
        "",
    ]
    for name, script in component_scripts:
        ...
        if name in probes:
            p = probes[name]
            lines.append(
                f"wait_ready {shlex.quote(name)} {shlex.quote(p.kind)} "
                f"{shlex.quote(p.target)} {p.timeout_seconds} {p.interval_seconds}"
            )
        lines.append("")
    return "\n".join(lines) + "\n"
...
def materialize_benchmark_stack_plan(
    ...
    *,
    ...
    # Validate proxy components have real commands (not literal placeholders)
    placeholder_proxy_components = [
        component.name
        for component in stack_plan.components
        if component.kind == "proxy" and "<path-to-" in component.command
    ]
    if placeholder_proxy_components:
        ...
        raise ValueError(
            ...
            f"{names}. Rebuild the stack plan with a concrete proxy command."
        )
    ...
    # Collect readiness probes keyed by component name for start script generation
    readiness_probes = {
        component.name: component.readiness
        for component, _ in component_script_refs
    ...
    start_script.write_text(
        _render_start_script(
            [(component.name, script_path) for component, script_path in component_script_refs],
...
def build_benchmark_stack_plan(
    ...
    *,
    ...
    if experiment.engine == "vllm" and experiment.topology.mode == "prefill_decode_split":
        ...
        # Dynamo: primary metrics = decode worker (vLLM), router is supplemental orchestration telemetry
        metrics_endpoint = f"http://{host}:{ports['decode']}"
        metrics_target_overrides = {
            "router": f"http://{host}:{ports['router']}",
            "prefill": f"http://{host}:{ports['prefill']}",
            "decode": f"http://{host}:{ports['decode']}",
        }
    else:
    ...
    if experiment.engine == "vllm" and experiment.topology.mode == "single_endpoint":
        ...
        component = LaunchComponent(
            ...
            readiness=LaunchReadinessProbe(
                kind="http",
                target=f"http://{host}:{ports['primary']}/health",
                timeout_seconds=180,
            ),
        ...
        # Legacy: SiMM remote_backend branches removed. Use Dynamo + NIXL for disaggregation.
        if is_lmcache:
        ...
        components.extend(
            [
                LaunchComponent(
                    ...
                    readiness=LaunchReadinessProbe(
                        kind="http",
                        target=f"http://{host}:{ports['prefill']}/health",
                        timeout_seconds=180,
                    ),
                    required_binaries=["vllm"],
                ),
                LaunchComponent(
                    ...
                    readiness=LaunchReadinessProbe(
                        kind="http",
                        target=f"http://{host}:{ports['decode']}/health",
                        timeout_seconds=180,
                    ),
                ...
                LaunchComponent(
                    ...
                    command=(
                        ...
                        else "python3 -m vllm.entrypoints.openai.api_server_disagg "
                    ) + (
                    ...
                    endpoint=request_endpoint,
                    notes=[
                        "vLLM disaggregated proxy routes requests to prefill/decode workers.",
                        "Send benchmark traffic to the proxy endpoint, not directly to prefill/decode workers.",
                        "If the default entrypoint is unavailable, pass --vllm-proxy-command to override.",
                    ],
                    warnings=[]
                    if vllm_proxy_command
                    else [
                        "Using default vLLM disagg entrypoint (vllm.entrypoints.openai.api_server_disagg). "
                        "If this module does not exist in your vLLM version, pass --vllm-proxy-command."
                    ],
                    readiness=LaunchReadinessProbe(
                        kind="tcp",
                        target=f"{host}:{ports['primary']}",
                        timeout_seconds=60,
                    ),
        ...
        sglang_command = _append_command_args(
            engine_config.command,
            "--host",
            host,
            "--port",
        ...
        components.append(
            LaunchComponent(
                ...
                readiness=LaunchReadinessProbe(
                    kind="http",
                    target=f"http://{host}:{ports['router_metrics']}/metrics",
                    timeout_seconds=60,
                ),
                depends_on=["sglang-prefill", "sglang-decode"],
                required_binaries=["sglang"],
            )
        )
        if not prefill_memory.fits:
        ...
        # Dynamo 1.0 orchestrated prefill/decode disaggregation with NIXL KV transfer.
        # Dynamo acts as the orchestration layer; vLLM (or SGLang) workers do the actual inference.
        # The Smart Router + SLO Planner handle request routing and KV-aware scheduling.
        generated_files.extend(_dynamo_config_files(
            host, ports, request_id_headers=request_id_headers,
        ))

        # Compile worker engines (vLLM by default under Dynamo orchestration)
        worker_engine = "vllm"
        prefill_config, prefill_memory = _compile_engine(
            selected_model, gpu, len(gpu_layout["prefill"]), workload_mode, worker_engine,
        )
        decode_config, decode_memory = _compile_engine(
            selected_model, gpu, len(gpu_layout["decode"]), workload_mode, worker_engine,
        )

        # Workers first, then router — workers must register before router starts routing.
        components.extend([
            LaunchComponent(
                name="vllm-prefill",
                role="prefill",
                ...
                gpu_ids=gpu_layout["prefill"],
                notes=list(prefill_config.notes) + [
                    "Prefill worker under Dynamo orchestration — registered with Smart Router.",
                    "NIXL handles KV transfer to decode workers (no manual kv_connector config needed).",
                ],
                warnings=list(prefill_config.warnings),
                readiness=LaunchReadinessProbe(
                    kind="http",
            ...
            LaunchComponent(
                ...
                gpu_ids=gpu_layout["decode"],
                notes=list(decode_config.notes) + [
                    "Decode worker under Dynamo orchestration — registered with Smart Router.",
                    "Receives KV cache from prefill workers via NIXL (RDMA/NVLink).",
                ],
                warnings=list(decode_config.warnings),
                readiness=LaunchReadinessProbe(
                    kind="http",
            ...
            LaunchComponent(
                ...
                command=(
                    f"dynamo serve "
                    f"--config configs/dynamo-config.yaml "
                    f"--host {host} --port {ports['primary']} "
                    f"--router-port {ports['router']}"
                ),
                endpoint=request_endpoint,
                metrics_endpoint=f"http://{host}:{ports['router']}",
                notes=[
                    "Dynamo 1.0 Smart Router with KV-aware scheduling and SLO Planner.",
                    "Send benchmark traffic to this endpoint — it routes to prefill/decode workers.",
                    "NIXL handles KV cache transfer between prefill and decode pools via RDMA/NVLink.",
                ],
                readiness=LaunchReadinessProbe(
                    kind="http",
                    target=f"http://{host}:{ports['router']}/metrics",
                    timeout_seconds=60,
                ),
                depends_on=["vllm-prefill", "vllm-decode"],
                required_binaries=["dynamo"],
            ),
        ])
        # Find the workers we just appended to apply memory warnings
        for c in components:
            if c.name == "vllm-prefill" and not prefill_memory.fits:
                c.warnings.append("Model does not fit on the prefiller GPU slice")
                warnings.append("Dynamo prefiller worker does not fit the selected model/GPU plan")
            if c.name == "vllm-decode" and not decode_memory.fits:
                c.warnings.append("Model does not fit on the decoder GPU slice")
                warnings.append("Dynamo decoder worker does not fit the selected model/GPU plan")
        notes.extend([
            "Dynamo 1.0 orchestrates the prefill/decode split with NIXL KV transfer.",
            "Workers register with the Dynamo Smart Router automatically on startup.",
            "SLO Planner adjusts routing based on real-time latency and KV cache pressure.",
            "Use NVLink or RDMA fabric for production-grade NIXL transfer performance.",
        ])
    elif experiment.engine == "dynamo" and experiment.topology.mode == "single_endpoint":
        # Single-endpoint Dynamo uses minimal config; pass ports for consistency
        generated_files.extend(_dynamo_config_files(host, ports, request_id_headers=request_id_headers))
        engine_config, memory_plan = _compile_engine(
            selected_model,
            gpu,
            num_gpus,
