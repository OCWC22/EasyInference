...
```bash
# Aggregate raw results and compute metrics
isb1 analyze --input results/raw --output results/aggregated

# Evaluate claims against measured data
...
- `--config-root PATH` — Root directory for config files (default: `configs/`).
...
**Options:**
- `--sweep PATH` — Path to the sweep configuration YAML.
...
```bash
isb1 run --sweep configs/sweep/core.yaml --output results/
isb1 run --sweep configs/sweep/extended.yaml --output results/ --dry-run
```
...
**Options:**
- `--gpu NAME` — GPU short name (e.g., `h100`, `h200`, `b200`, `b300`).
- `--model NAME` — Model short name (e.g., `llama70b`, `dsr1`).
...
- `--output PATH` — Output directory for results.
- `--trials N` — Number of trials (default: from sweep config).
...
**Options:**
- `--input PATH` — Directory containing raw benchmark results.
- `--output PATH` — Directory for aggregated metrics output.
- `--format FORMAT` — Output format: `json`, `csv`, or `both` (default: `both`).

### `isb1 claims`

Evaluate performance claims against measured data.
...
**Options:**
- `--input PATH` — Directory containing aggregated metrics.
- `--output PATH` — Directory for claim evaluation reports.
- `--claims-file PATH` — Optional YAML file defining specific claims to evaluate.

### `isb1 leaderboard`

Generate ranked leaderboard tables.
...
**Options:**
- `--input PATH` — Directory containing aggregated metrics.
- `--output PATH` — Directory for leaderboard output.
